  ...
  return {
    ...
    execute: function () {
      ...
      _export("Createnew", Createnew = (_dec = ccclass("Createnew"), _dec2 = property(Button), _dec3 = property(Prefab), _dec(_class = (_class2 = class Createnew extends Component {
        ...
        onLoad() {
          var strlabel = this.node.getChildByName("Tip").getComponent(Label);

          var str = (_crd && GameManager === void 0 ? (_reportPossibleCrUseOfGameManager({
            error: Error()
          ...
          var cpint2 = (_crd && GameManager === void 0 ? (_reportPossibleCrUseOfGameManager({
            error: Error()
          }), GameManager) : GameManager).inst()._Money;

          for (var i = 1; i <= this.Loacthousecount; i++) {
            this.CreateShelf();
          }
        }
        ...
        CreateShelf() {
          if (this.currentHouseCount === this.MaxhouseCount) {
            var panel = this.node.getChildByName("MessagePanel");
            panel.active = true;
            return false;
          }

          var bool = this.AddWareHouse(this.currentHouseCount + 1);
        ...
        AddWareHouse(num) {
          try {
            ...
            var quotient = Math.floor(num / 2); // 商

            var remainder = num % 2; // 余数
            // 根据余数设置 newx

            newx = remainder === 0 ? 180 : -180; // 根据 quotient 计算 newy 的位置

            newy = this.Y - (quotient + remainder) * 200;
            var house = instantiate(this.House);
            house.setPosition(v3(newx, newy, 0)); // 设置房屋位置

            var LAB = house.getChildByName("Title").getComponent(Label);
            LAB.string = "鲜花";
...
//# sourceMappingURL=498f547f83283d0cc4a6b9b51186dcdc6bda36a0.js.map
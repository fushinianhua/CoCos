      ...
      _export("Move", Move = (_dec = ccclass("Move"), _dec2 = property(Vec2), _dec3 = property(Vec2), _dec4 = property(Prefab), _dec5 = property(Node), _dec(_class = (_class2 = class Move extends Component {
        ...
        start() {
          this.scheduleOnce(() => {
            ...
            let FutureWorkLabel = instantiate(this.FutureWorkLabel); // 设置标签的位置

            FutureWorkLabel.setParent(this.node); // 动画效果

            let t1 = tween(FutureWorkLabel).to(1.5, {
              position: new Vec3(0, 95, 0)
            }, {
              easing: "backInOut"
            }); // 移动节点效果

            let t2 = tween(this.node).to(2, {
        ...
        onTouchStart(event) {
          //触摸位置
          const touchLocation = event.getUILocation(); //结点位置

          const nodeLocation = this.node.getPosition(); //计算位置偏移量
        ...
        onTouchMove(event) {
          ...
          let newPosition = new Vec3(touchLocation.x - this._offset.x, touchLocation.y - this._offset.y, this.node.getPosition().z); // 限定拖动范围
        ...
        clampPosition(position) {
          // 将位置x限制在最小边界和最大边界之间
          const clampedX = Math.max(this.minBoundary.x, Math.min(position.x, this.maxBoundary.x)); // 将位置y限制在最小边界和最大边界之间

          const clampedY = Math.max(this.minBoundary.y, Math.min(position.y, this.maxBoundary.y)); // 返回新的Vec3对象，其中x和y被限制在边界内，z保持不变
